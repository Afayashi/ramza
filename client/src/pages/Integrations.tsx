/*
 * التكاملات - رمز الإبداع
 * صفحة إدارة التكاملات مع الخدمات الخارجية
 */
...
function IntegCard({ icon, name, desc, status, features, actionLabel, onAction, actionLoading, docsUrl, note, disabled }: IntegCardProps) {
      ...
      <div className="px-5 pb-5 flex gap-2">
        <button
          ...
          className={`flex-1 py-2.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition disabled:opacity-60 ${
            isConnected ? 'bg-gray-100 text-gray-700 hover:bg-gray-200' : 'text-white'
          }`}
...
export default function Integrations() {
  ...
  const fetchUser = async () => {
    try { setUser(await (base44 as any).auth.me()); }
    catch { /* not authenticated via base44 */ }
    ...
    if (user?.ejarToken) {
      ...
      try {
        await (base44 as any).auth.updateMe({
          ejarUsername: null, ejarLicenseNumber: null,
          ejarToken: null,    ejarConnectedAt: null,
        });
        setUser((p: any) => ({ ...p, ejarToken: null }));
      } catch { /* ignore */ }
  ...
  const handleEjarSync = async () => {
    setSyncing(true);
    try { await (base44 as any).functions.invoke('syncEjarContracts', {}); }
    catch { /* ignore */ }
    setSyncing(false);
  };
